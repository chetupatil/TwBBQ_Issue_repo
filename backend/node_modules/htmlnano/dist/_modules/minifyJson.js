...
const mod = {
    onContent () {
        return (content, node)=>{
            ...
            const nodeType = node.attrs && typeof node.attrs.type === 'string' ? helpers.normalizeMimeType(node.attrs.type) : undefined;
            if (nodeType && rNodeAttrsTypeJson.test(nodeType)) {
                try {
                    ...
                    return [
                        JSON.stringify(JSON.parse(jsonContent)).replace(/</g, '\\u003C')
                    ];
                } catch  {
                // Invalid JSON
                }

...
/** Minify CSS with cssnano */ const mod = {
    async default (tree, _, cssnanoOptions) {
        const cssnano = await helpers.optionalImport('cssnano');
        const postcss = await helpers.optionalImport('postcss');
        if (!cssnano || !postcss) {
            return tree;
        }
        ...
        tree.walk((node)=>{
            ...
            if (helpers.isStyleNode(node) && helpers.isCssStyleType(node)) {

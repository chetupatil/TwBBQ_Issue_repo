...
var normalizeAttrsForKey = require('../normalizeAttrsForKey-mqe76fnv.js');
...
function normalizeScriptAttrsForKey(attrs, scriptType) {
    return normalizeAttrsForKey.normalizeAttrsForKey(attrs, {
        baseAttrs: {
            type: scriptType
        },
        booleanAttrs,
...
// to leave them alone.
function collectScriptNodes(node, scriptNodes) {
...
function mergeScriptNodes(scriptNodesIndex, tracking) {
    for (const scriptNodes of Object.values(scriptNodesIndex)){
        ...
        scriptNodes.reverse().forEach((scriptNode)=>{
            let scriptContent = helpers.extractTextContentFromNode(scriptNode).trim();
            if (!scriptContent) {
...
/* Merge multiple <script> into one */ const mod = {
    default (tree) {
        ...
        const isolatedScriptNodes = new WeakSet();
        tree.walk((node)=>{
            if (normalizeAttrsForKey.isIsolatedScope(node)) {

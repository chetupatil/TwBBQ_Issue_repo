...
function processStyleNodeUnCSS(html, styleNode, uncssOptions, uncss) {
    const css = helpers.extractCssFromStyleNode(styleNode);
    ...
    return runUncss(html, strippedCss, uncssOptions, uncss).then((css)=>{
        ...
        styleNode.content = [
            helpers.wrapCssCdata(css, isCdataWrapped)
        ];
    });
}
...
/** Remove unused CSS */ const mod = {
    async default (tree, options, userOptions) {
        ...
        let html;
        let extractor;
        const purgecss = await helpers.optionalImport('purgecss');
        const uncss = await helpers.optionalImport('uncss');

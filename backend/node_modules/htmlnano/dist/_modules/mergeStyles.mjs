...
import { i as isIsolatedScope, n as normalizeAttrsForKey } from '../normalizeAttrsForKey-h103i6n9.mjs';
...
function mergeStylesInScope(nodes, group) {
    nodes.forEach((node, index)=>{
        ...
        if (isIsolatedScope(node)) {
            ...
            if (Array.isArray(node.content)) {
                mergeStylesInScope(node.content, {
                    key: null,
                    node: null
                });
            }
        ...
        if (group.node && group.key === styleKey) {
            var _group_node, _content;
            const styleContent = extractStyleTextContent(node);
            (_content = (_group_node = group.node).content) != null ? _content : _group_node.content = [];

...
var _modules_removeRedundantAttributes = require('./removeRedundantAttributes.js');

/** Minify JS with Terser */ const mod = {
    async default (tree, _, terserOptions) {
        const terser = await helpers.optionalImport('terser');
         ...
         */ if ('integrity' in nodeAttrs) {
            ...
            if (node.tag && node.tag === 'script') {
                const rawMimeType = typeof nodeAttrs.type === 'string' ? helpers.normalizeMimeType(nodeAttrs.type) : undefined;
                const mimeType = rawMimeType || 'text/javascript';
                if (_modules_removeRedundantAttributes.redundantScriptTypes.has(mimeType) || mimeType === 'module') {
...
function analyzeTreeQuotes(tree) {
    ...
    tree.walk((node)=>{
        ...
        for (const [attrName, attrValue] of Object.entries(node.attrs)){
            ...
            if (hasDoubleQuote && helpers.isEventHandler(attrName)) {
...
function processScriptNode(scriptNode, terserOptions, terser) {
    let js = helpers.extractTextContentFromNode(scriptNode).trim();
    if (!js.length) {
        return;
    }
...
function processNodeWithOnAttrs(node, terserOptions, terser) {
    ...
    for(const attrName in node.attrs){
        if (!helpers.isEventHandler(attrName)) {
            continue;
        }
